...
最大的工程挑戰是\textbf{真實校園路網}從哪裡來。校內的 22 個地標可以手寫，但「校門口走到綜合體育館」要經過哪些 OSM \texttt{highway=footway} 才不會穿牆？我們用 \texttt{scripts/build-dense-graph.mjs} 透過 Overpass API 抓取台大校園 bounding box 內所有 \texttt{highway} tagged ways。
...
\textbf{Dijkstra 實作}：使用 priority queue 的 $O((V+E)\log V)$ 標準版本，但在 cost function 中即時讀取「目前 active 的 \texttt{flooding} / \texttt{standing\_water} 回報」並施加懲罰。\textbf{亦即：使用者剛在地圖回報的積水，下一個雨天規劃路徑的同學就會自動繞開。}
...
\textbf{AI 管理分析層}則建立在上述可解釋分數之上，而不是取代它。系統先由 \texttt{dashboardAi.ts} 產生 deterministic fallback：週比異常、熱點集中、逾期案件、飲水機健康度下降、維修優先序與 CSV 欄位；接著才交給 Gemini API 改寫成管理者可讀的摘要與月報。這樣兼顧了 AI 的文字整理能力與工程系統的穩定性。
...
Gemini API & AI 摘要生成 & \texttt{/api/dashboard/ai-insights} server-side 呼叫，將儀表板統計、異常偵測與維修優先序改寫成管理摘要；無 \texttt{GEMINI\_API\_KEY} 時使用本機 fallback。\\
\midrule
