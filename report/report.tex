...
\begin{tikzpicture}
\node[circle,fill=brand,minimum size=2.3cm] (logo) {};
...
{\color{mute}\large 校園即時回報、雨天導航、水災預警一站式平台}\\[1.35cm]

\begin{tcolorbox}[
  enhanced, colback=paper, colframe=brand!30,
  arc=3mm, boxrule=0.6pt, width=0.88\linewidth,
  left=16pt,right=16pt,top=11pt,bottom=11pt,
]
\renewcommand{\arraystretch}{1.35}
\begin{tabular}{@{}l p{10cm}@{}}
\textbf{\color{brandDeep}課程} & 水資源概論 (BSE1011)\\
\textbf{\color{brandDeep}類型} & Web Application — 互動式校園地圖平台\\
\textbf{\color{brandDeep}資料源} & OpenStreetMap、中央氣象署、台北自來水處\\
\textbf{\color{brandDeep}組別} & 第 31 組 \\
\textbf{\color{brandDeep}組員與分工表} &
{\normalsize
\renewcommand{\arraystretch}{1.05}
\begin{tabular}[t]{@{}p{4.8cm}p{5.9cm}@{}}
資管二 B13705013 劉起佑 & 前端介面、互動地圖流程 \\
...
\end{tabular}} \\
\textbf{\color{brandDeep}Github Repo} &
\href{https://github.com/CdyTim630/ntu_water_map}
{\texttt{github.com/CdyTim630/ntu\_water\_map}} \\
\textbf{\color{brandDeep}網站} &
\href{https://ntuwatermap.vercel.app/}{ntuwatermap.vercel.app} \\
\end{tabular}
\end{tcolorbox}
...
\emph{社群回報}、\emph{即時氣象}、\emph{校園路網最佳化}與\emph{數據儀表板}，把分散在校園各角落的積水、漏水、淹水資訊集中視覺化，並進一步轉化為「下一步該怎麼走、該怎麼避」的具體建議。

不同於傳統紙本問卷或單向公告，本平台採取\textbf{雙向數據迴圈}：使用者回報的問題會即時影響其他人的雨天路徑規劃；\emph{校方管理端}可從儀表板、AI 管理摘要與 CSV 匯出觀察高風險點，作為改善決策依據。
全站共 6 個主要頁面、11 個 Node.js API Routes，整合 3 項外部公開資料源與 Gemini AI API，路徑規劃部分以 Dijkstra（雙層 GeoJSON 路網，6{,}133 節點、7{,}248 條邊）即時計算最低淋雨成本。
...
\item \textbf{社群飲水機地圖}：融合 OSM 社群 + 北水處官方 OpenData，26 台校內飲水機可即時報修、+1 減塑
...
\begin{enumerate}
\item \textbf{回報沒人理}：傳統表單填了之後不知道有沒有人看 $\Rightarrow$ 用\emph{社群確認機制（still\_exists / resolved）}讓使用者相互驗證。
\item \textbf{雨天還是用 Google Maps}：但 GMaps 不知道椰林大道哪段有騎樓 $\Rightarrow$ 用\emph{OSM 抓的密集路網} + \emph{雨勢加權 Dijkstra} 給出真正避雨的走法。
\item \textbf{看數據的人不在地圖前}：校方需要的是趨勢與排行，也需要一句能直接決策的摘要 $\Rightarrow$ 用\emph{14 天趨勢 + 風險分數排行 + 處理 SLA + AI 管理分析} 的儀表板。
\end{enumerate}
\end{warnbox}
...
\node[lanetag=brandDeep!70]      at (16.7, \yApi)    {API \\ \scriptsize\color{mute}\textmd{11 endpoints}};
...
\node[api] (apibox) at (7.95, \yApi) {%
  \textbf{Next.js API Routes}（11 endpoints, server-side）\\[1pt]
  \texttt{/api/\{reports, route, forecast, weather, stats, dashboard/ai-insights, water-stations\}}
};

% ── Layer 3: Algorithm ──
\node[algo] (algobox) at (7.95, \yLogic) {%
  \textbf{演算法層}\;\;Dijkstra $\cdot$ Risk Score $\cdot$ 90 天熱力預測 $\cdot$ Gemini 摘要 / 本機 fallback
};
...
\end{tikzpicture}
\caption{系統五層架構（Swim-lane 視覺化）：Client 6 個頁面 → API Routes 11 endpoints → 演算法層 → 內部資料（GeoJSON / Mock / LocalStorage）→ 外部資料源（OSM / 北水處 / CWA）與 Gemini AI API。實線=內部呼叫，虛線=外部 API。\texttt{/me} 直連 LocalStorage 不經 server 以保護隱私。}
\end{figure}
...
\item \textbf{回報社群確認}：每筆 report 支援「\emph{還在}」「\emph{已解決}」雙向投票，自動更新 \texttt{upvote\_count} / \texttt{resolved\_count}。
\end{itemize}
\end{infobox}
...
最新版本進一步加入 \textbf{AI 管理分析助理}：\texttt{/api/dashboard/ai-insights} 會先用本機規則引擎計算可信的異常偵測與維修優先序，再於 server-side 呼叫 Gemini API 產生管理摘要。若沒有設定 \texttt{GEMINI\_API\_KEY} 或 Gemini 暫時失敗，系統會自動 fallback 到本機分析，因此 demo 不會因外部 AI 服務中斷而空白。
...
\begin{infobox}
\textbf{\color{brandDeep}儀表板核心 KPI 與 AI 分析（給校方決策層看）}
\begin{itemize}
...
\item \textbf{AI 管理摘要}：自動生成「本週回報增加、集中地點、異常原因、建議處理順序」等自然語言說明，並輸出可下載的維修優先序 CSV。
\end{itemize}
\end{infobox}
...
設計動機：\textbf{回報意願是專案存活的關鍵}。我們把抽象的「\emph{做好事}」轉化為立即可見的回饋 —— 連續打卡天數（Streak）、行動分數（4 維加權）、徽章解鎖、社群分享文與可下載的個人成績單。
14 枚徽章橫跨「\textbf{連續打卡}」「\textbf{飲水機 / 減塑}」「\textbf{通勤路線}」「\textbf{校園回報}」四個行為類別，從新手「\emph{初次入坑}」到極限「\emph{一年水文皇帝}」（連續打卡 365 天），涵蓋短期與長期動機。
最新版本加入 \textbf{Share Report Card}：使用者可將目前成績轉成 1080 $\times$ 1350 的社群分享圖、匯出 PDF，或用 Web Share API 分享文字。若使用者尚未累積足夠本機紀錄，頁面會先顯示一組 demo profile，讓分享功能在期末展示時不會是空白狀態；一旦開始打卡、裝水或回報，即自動切回真實 LocalStorage 資料。

\begin{figure}[H]\centering
\includegraphics[width=0.96\linewidth]{screens/me.png}
\caption{個人成績單畫面：段位（水文學徒 → 水文大師）、4 格累積成績（連續打卡、飲水機+1、通勤路線、校園回報）、14 枚徽章牆與分享卡。所有真實使用紀錄純本機儲存（LocalStorage），零追蹤。}
\end{figure}
...
路網由 \texttt{scripts/build-dense-graph.mjs} 透過 Overpass API 抓取台大 bounding box 內所有 \texttt{highway} ways，再轉成 Dijkstra 可用的 graph。資料分成兩層：\texttt{campus.geojson} 保存 22 個具名地標供 UI 選單使用；\texttt{campus-paths.geojson} 保存 6{,}133 節點、7{,}248 邊的密集路網（$\approx$3 MB），只在 server-side 載入，避免拖慢 client。
...
\textbf{Dijkstra 實作}使用 priority queue，時間複雜度為 $O((V+E)\log V)$。cost function 同時讀取 OSM 遮蔽標籤（\texttt{indoor/tunnel/covered/arcade}）、雨勢係數 $r$，以及 active 的 \texttt{flooding} / \texttt{standing\_water} 回報；雨越大，系統越願意繞去有騎樓或室內走廊的路。中度雨勢（$r=0.6$）下，防雨路徑相較最短路徑可提升遮蔽率 $\sim$30\,\%，僅多走 $\sim$50 m；$r=0$ 時兩者重疊。
...
分數 $\geq 9$ 標記為\textcolor{danger}{\textbf{高風險}}、$\geq 4$ 為\textcolor{warn}{\textbf{中風險}}，其餘為低風險。AI 管理分析層不取代此規則，而是先由 \texttt{dashboardAi.ts} 產生 deterministic fallback（異常、熱點、逾期案件、飲水機健康度、維修優先序與 CSV 欄位），再交給 Gemini API 改寫成管理摘要與月報。
...
本平台整合\textbf{三項外部公開資料}、\textbf{一項外部 AI API} 與\textbf{兩項內部資料}：

\begin{table}[H]
\centering
\footnotesize
\renewcommand{\arraystretch}{1.16}
\begin{tabularx}{\linewidth}{@{}p{2.75cm}p{2.65cm}X@{}}
\toprule
\textbf{資料源} & \textbf{類型} & \textbf{用途與處理}\\
\midrule
OpenStreetMap & 地理路網（Overpass） & 抓取校內 \texttt{highway=footway/path/service} 產生 6{,}133 節點路網，並以 \texttt{indoor/tunnel/arcade} 推論遮蔽率。\\
中央氣象署 OpenData & 即時氣象 & 觀測 \texttt{O-A0003-001} 與預報 \texttt{F-D0047-061} 提供雨量、降雨機率、溫溼度；無 \texttt{CWA\_API\_KEY} 時 fallback 至 mock。\\
北水處 OpenData & 飲水機水質 & 與 OSM \texttt{amenity=drinking\_water} 融合（\texttt{source: osm / official / merged}），保留採樣時間與官方狀態。\\
Gemini API & AI 摘要生成 & \texttt{/api/dashboard/ai-insights} server-side 呼叫，將統計、異常與維修優先序改寫成管理摘要；無 key 時使用本機 fallback。\\
\midrule
內建 Mock Store & In-memory & demo 模式預設啟用，無 Supabase 也可跑通；冷啟動會 reset。\\
LocalStorage（client） & 個人成績 & 連續打卡、行動分數、徽章狀態留在本機，零追蹤。\\
\bottomrule
\end{tabularx}
\caption{資料源整合一覽。所有外部 API 都由 server-side 代理（\texttt{/api/weather}、\texttt{/api/water-stations}、\texttt{/api/dashboard/ai-insights}），API key 不暴露在 client。}
\end{table}
...
% \begin{itemize}
% \item \textbf{11 個 API Routes 全 Node runtime}：明確排除 GitHub Pages 等純靜態託管的可能性；選 Vercel hnd1 取得最低台日 RTT。
...
% \texttt{/api/reports/[id]/confirm} & POST & 社群確認（\texttt{still\_exists} / \texttt{resolved}）\\
% \texttt{/api/stats} & GET & 儀表板聚合（KPI + 趨勢 + 排行 + 飲水機 SLA）\\
% \texttt{/api/dashboard/ai-insights} & GET & Gemini / fallback 管理摘要、異常偵測、維修優先序與 CSV rows\\
...
不只是一般課堂作業 demo，而是已部署於 Vercel、有 6 個頁面、11 個 API、整合 3 個外部公開資料源與 Gemini AI API 的\emph{產品級實作}。
...
\item \textbf{AI 分析擴充}：目前 Gemini 主要負責管理摘要與維修優先序；未來可加入 embedding 合併相似回報、偵測重複案件，並讓月報自動附上「本月改善前後」對照。
...
\url{https://www.water.gov.taipei/Content_List.aspx?n=77AB4A57F76B9882}
